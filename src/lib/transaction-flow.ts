 ...
 * Uses the same resolver as list UI badges (`transactionEntryType`): persisted
 * `transaction_type` when set; otherwise category heuristic (legacy rows).
 */
 ...
 * Same resolver as UI so analytics match what users see per row.
 */

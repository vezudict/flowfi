...
| `src/lib/ai-insights.ts` | **`generateAIInsights(summary)`** — OpenAI **Chat Completions** (`/v1/chat/completions`) with **`model`** = `process.env.OPENAI_MODEL` or **`gpt-4o-mini`** (server log `AI MODEL USED` on each OpenAI call); `temperature: 0.4`; fixed system/user messages; returns **`AIInsight[]`** (max 6). **429 / rate limit:** returns static fallback insights (no throw); other errors **`OPENAI ERROR`** log then rethrow. Server-only | `api/ai-insights` route |

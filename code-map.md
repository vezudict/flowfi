...
- **AI insights (user-controlled):** No auto-fetch on page load. Toggle switch (`aiEnabled` state) above `FinancialInsights` card controls activation. **`handleToggleAI`**: sets `aiEnabled`; if turning ON and no data yet, calls `fetchAIInsights()`. **`fetchAIInsights`**: manual trigger + regenerate button → `POST /api/ai-insights` → `ai_insights_cache` → `generateAIInsights` on cache miss; sets `aiLastUpdated` timestamp. When OFF: `aiInsights=null` passed to `FinancialInsights` (rule-based Spending section shows as fallback). When ON: `AIInsight[]` flows to `AIInsightSection`. Regenerate button + last-updated timestamp shown while enabled. Zero token usage until user explicitly enables.

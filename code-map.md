...
| `src/lib/ai-insights.ts` | **`generateAIInsights(summary, openaiModel?)`** — OpenAI **Chat Completions** (`/v1/chat/completions`) with **`model`** = `process.env.OPENAI_MODEL` or **`gpt-4o-mini`** (route: **`🔥 AI MODEL USED`** before OpenAI; lib: `model` param in Chat Completions body); `temperature: 0.4`; fixed system/user messages; returns **`AIInsight[]`** (max 6). **429 / rate limit:** returns static fallback insights (no throw); other errors **`OPENAI ERROR`** log then rethrow. Server-only | `api/ai-insights` route |
...
| `src/app/api/ai-insights/route.ts` | POST JSON body **`{ analytics, currency }`** (required). **Flow:** dashboard **`useEffect`** → this route → read **`ai_insights_cache`** (validates structured `AIInsight[]`; skips legacy `string[]` cache) → on miss logs **`🔥 AI MODEL USED`** (`OPENAI_MODEL` or **`gpt-4o-mini`**) → **`generateAIInsights(summary, MODEL)`** → upsert cache |

...
| `src/lib/ai-insights.ts` | **`generateAIInsights(summary, openaiModel?)`** — takes enriched **`InsightsSummary`** (includes `categoryBreakdown[]`, `unusualSpikes[]`, `avgDailySpend`, `transactionCount`, `previousMonthIncome`). **Heuristic guard:** if `transactionCount < 3` or `totalSpending < 0.01`, returns sparse fallback (no OpenAI call). OpenAI **Chat Completions** with `response_format: { type: "json_object" }`, `temperature: 0.4`; premium prompt — anti-generic rules, all insights must cite real numbers; expects `{ insights: AIInsight[] }` envelope; returns max **5** insights. **429:** static fallback; other errors rethrow. **Dev:** logs `AI CONTEXT` and `AI RAW RESPONSE`. Server-only | `api/ai-insights` route |
